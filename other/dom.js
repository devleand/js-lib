...
DOMElements.prototype = {
    ...
    clone: function (deep= true) {
        ...
        return DOMElements.make(els);
    },

    /**
     * @param {string} selector
     * @returns {DOMElements|null}
     */
    parent: function (selector) {
        let els = [];
        this.forEach(function (el) {
            let parent = el.closest(selector);
            if (Types.isNull(parent)) return null;
            els.push(parent);
        });
    ...
    /**
     * @param {string | DOMElements | NodeList | Element} nodes
     */
    appendChild: function (...nodes) {
        nodes = DOMElements.getAsList(nodes);
        this.forEach(function (el) {
